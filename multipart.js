 ...
 */
// Original source from https://github.com/nachomazzara/parse-multipart-data
// Required small modification to create default filename handler per part
  ...
  for (var i = 0; i < multipartBodyBuffer.length; i++) {
    ...
    if (ParsingState.INIT === state && newLineDetected) {
      ...
      if (lastline.length) {
        ...
        for (
          var _i = 0, currentPartHeaders_1 = currentPartHeaders;
          _i < currentPartHeaders_1.length;
          _i++
        ) {
...
function process(part) {
  ...
  Object.defineProperty(input, "name", {
    value: "dicom", //header[1].split('=')[1].replace(/"/g, ''),
    writable: true,
    enumerable: true,
    configurable: true,
  });
...
function process(part) {
  ...
  // always process the name field
  Object.defineProperty(input, "name", {
    value: getValueOrDefault(header[1], "filename=blank").split('=')[1].replace(/"/g, ''),
    writable: true,
    enumerable: true,
    configurable: true,
  });
...
function getValueOrDefault(s, def) {
    const blank = !s || s === '';
    return blank ? def : s;
}